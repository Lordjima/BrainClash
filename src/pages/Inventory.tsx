...
import { Package, Search, Filter, Trash2, Zap, Shield, Heart, Info, ShoppingBag, Backpack, ArrowLeft, Play, Star, User } from 'lucide-react';
...
export default function Inventory() {
  ...
  const { inventory, profile } = useUser();
  ...
  const inventoryItems = (inventory || []).map(entry => {
    const item = shopItems.find(item => item.id === entry.itemId);
    return item ? { ...item, entryId: entry.id } : null;
  }).filter(Boolean);
  ...
  const stats = {
    ...
    utility: inventoryItems.filter(i => i?.type === 'spell').length,
  };
  ...
  return (
    <PageLayout>
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-8 overflow-y-auto lg:overflow-hidden custom-scrollbar">
                  ...
                  {(() => {
                    ...
                    const filterOptions = [
                      ...
                      ...availableTypes.map(type => ({
                        ...
                        icon: type === 'attack' ? Zap : type === 'defense' ? Shield : type === 'bonus' ? Star : type === 'spell' ? Zap : Heart,
                        color: type === 'attack' ? 'text-red-500' : 
                               type === 'defense' ? 'text-blue-500' : 
                               type === 'bonus' ? 'text-green-500' : 
                               type === 'spell' ? 'text-yellow-500' : 'text-emerald-500'
                      }))
                    ];
          ...
          {/* Center: Items Grid */}
          <div className="flex-1 bg-zinc-900/20 backdrop-blur-md border border-white/5 rounded-2 p-8 overflow-y-auto custom-scrollbar relative">
                ...
                {filteredItems.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className={`w-16 h-16 rounded-[24px] flex items-center justify-center shadow-2xl relative z-10 transition-transform group-hover:scale-110 group-hover:rotate-3 ${
                      item.type === 'attack' ? 'bg-red-500/10 text-red-500' : 
                      item.type === 'defense' ? 'bg-blue-500/10 text-blue-500' : 
                      item.type === 'bonus' ? 'bg-green-500/10 text-green-500' : 
                      item.type === 'spell' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-emerald-500/10 text-emerald-500'
                    }`}>
                      {getIcon(item.icon, "w-8 h-8")}
                    </div>
                    
                    <div className="text-center relative z-10">
                      <div className="text-[11px] font-black uppercase tracking-tight line-clamp-1 group-hover:text-white transition-colors mb-2">{item.name}</div>
                      <Badge 
                        variant={
                          item.type === 'attack' ? 'red' : 
                          item.type === 'defense' ? 'blue' : 
                          item.type === 'bonus' ? 'green' : 
                          item.type === 'spell' ? 'yellow' : 'emerald'
                        }
                        size="sm"
                      >
                        {item.type}
              ...
              >
                ...
                <div className="flex-1 space-y-12 relative z-10">
                  <div className="flex items-center justify-between">
                    <Badge 
                      variant={
                        selectedItem.type === 'attack' ? 'red' : 
                        selectedItem.type === 'defense' ? 'blue' : 
                        selectedItem.type === 'bonus' ? 'green' : 
                        selectedItem.type === 'spell' ? 'yellow' : 'emerald'
                      }
                    >
                      {selectedItem.type}
                    </Badge>
                  ...
                  <div className="text-center space-y-8">
                    <div className={`w-48 h-48 mx-auto rounded-[56px] flex items-center justify-center shadow-2xl relative group ${
                      selectedItem.type === 'attack' ? 'bg-red-500/10 text-red-500' : 
                      selectedItem.type === 'defense' ? 'bg-blue-500/10 text-blue-500' : 
                      selectedItem.type === 'bonus' ? 'bg-green-500/10 text-green-500' : 
                      selectedItem.type === 'spell' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-emerald-500/10 text-emerald-500'
                    }`}>
                ...
                <div className="pt-12 space-y-5 relative z-10">
                  ...
                  <button 
                    onClick={async () => {
                      ...
                      await QuizService.removeFromInventory(selectedItem.entryId.toString());
