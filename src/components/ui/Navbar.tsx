...
export default function Navbar() {
  ...
  const { twitchUser, isAdmin } = useAuth();
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'TWITCH_AUTH_SUCCESS' && event.data?.user) {
        ...
        <div className="flex items-center gap-6">
          {twitchUser ? (
              ...
              >
                <div className="relative">
                  <img
                    ...
                    alt="Profile"
                    className="w-10 h-10 rounded-full border-2 border-white/10 group-hover:border-fuchsia-500/50 transition-colors"
                ...
                <ChevronDown
                  className={`w-4 h-4 text-zinc-500 transition-transform duration-300 ${
                    isMenuOpen ? 'rotate-180 text-fuchsia-500' : ''
                  }`}
                />
              </button>
            ...
            <button
              ...
              className="flex items-center gap-2 bg-fuchsia-600 hover:bg-fuchsia-500 text-white px-6 py-2.5 rounded-full font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-fuchsia-600/20"
            >
