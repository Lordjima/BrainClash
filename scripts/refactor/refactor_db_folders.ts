...
// 1. core.ts
const coreText = `import mysql from 'mysql2/promise';
...
// 2. users.ts
const usersText = `import { query } from './core';
...
// 3. questions.ts
const questionsText = `import { query } from './core';
...
// 4. shop.ts
const shopText = `import { query } from './core';
...
// 5. index.ts
const indexText = `export * from './core';
export * from './users';
export * from './questions';
export * from './shop';
`;

fs.writeFileSync('src/lib/db/core.ts', coreText);
fs.writeFileSync('src/lib/db/users.ts', usersText);
fs.writeFileSync('src/lib/db/questions.ts', questionsText);
fs.writeFileSync('src/lib/db/shop.ts', shopText);
fs.writeFileSync('src/lib/db/index.ts', indexText);

// Now delete the old db.ts
fs.unlinkSync('src/lib/db.ts');

console.log('Database refactored successfully.');
